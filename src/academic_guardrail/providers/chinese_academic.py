        ...
        async with sem:
            try:
                ...
                if doi:
                    ...
                    if (openalex_res and openalex_res.get("title")) or (crossref_res and crossref_res.get("title")) or (clean_doi in KNOWN_RETRACTED_DOIS):
                        return {
                            "matched": True,
                            "doi": clean_doi,
                            "title": matched_title,
                ...
                if query_str and len(query_str) >= 3:
                    openalex_res = await self.openalex.search_by_title(query_str)
                    ...
                    s2_res = await self._search_semantic_scholar(query_str)
                    ...
                    cross_title_res = await self._search_crossref_title(query_str)
                    if cross_title_res and cross_title_res.get("doi"):
                        matched_title = cross_title_res.get("title", "")
                        if self._is_chinese_title_match(query_str, matched_title):
                            # Attempt to enrich abstract from OpenAlex/S2 via DOI
                            doi = cross_title_res.get("doi")
                            abstract = ""
                            if doi:
                                o_res = await self.openalex.get_by_doi(doi)
            ...
            CHINESE_CORE_JOURNALS = ["管理世界", "经济研究", "金融研究", "软件学报", "计算机学报", "中国社会科学", "法学研究", "世界经济", "会计研究", "中国工业经济", "数理统计与管理", "情报学报"]
            core_title = self._extract_core_title(title)
